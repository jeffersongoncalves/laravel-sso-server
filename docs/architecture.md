...
This document defines the cryptographic architecture, communication protocol, and integration contract implemented in `jeffersongoncalves/laravel-sso-server` (v1.0.x). Clients such as `jeffersongoncalves/laravel-sso-client` must follow it exactly.
...
Error bodies are `{"error": "...", "error_description": "..."}`. There is no refresh token in 1.0.x.

**JWT claims:** `iss` (`sso-server.issuer`, default the server's `APP_URL`), `sub` (user id, always a string), `aud` (`client_id`), `iat`, `nbf`, `exp`, `jti`, plus the claims returned by the configured `SsoUserSerializerContract` (default `name`, `email`). Reserved claims always win over serializer claims.
...
Triggered by `Illuminate\Auth\Events\Logout` on the configured guard (`sso-server.guard`), or manually with `SsoServer::logoutUser($userId)`:
...
The server has no client-initiated logout endpoint in 1.0.x.
